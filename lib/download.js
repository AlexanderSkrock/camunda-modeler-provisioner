...
function downloadPlugins (config) {
    const cache = new Cache(config.getCachePath());
    return Promise.all(config.getExternalPlugins().map(pluginUrl => {
        const fileName = basename(pluginUrl);
        return Promise.all([
            cache.isCached(fileName),
            config.shouldSkipCache() ? Promise.reject(new Error('cache may not be used')) : Promise.resolve(),
        ]).catch(() => {
            return downloadToCache(cache, fileName, pluginUrl);
...
function downloadCamundaModeler (config) {
    ...
    return getDownloadLink(config.getPlatform(), config.getVersion()).then(link => {
        ...
        return downloadPlugins(config).then(pluginFiles => {
            return {
                modeler: modelerFile,
                plugins: pluginFiles,
            };
