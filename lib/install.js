    ...
    return FileType.fromFile(zipPath).then(fileType => {
        if (fileType.mime) {
            if (fileType.mime === 'application/zip') {
                ...
                // By setting strip 1 we disabled extracting into subdirectory within the target path
                // For tar we have to ensure the target directory exists already
                return mkdir(targetPath, { recursive: true }).then(() => tar.extract({ file: zipPath, cwd: targetPath, strip: 1 }));
            }
        }
...
function extractPluginTo (installationPath, pluginPath) {
    ...
    return mkdir(pluginsPath, { recursive: true }).then(() => {
            ...
            info(`Extracting plugin "${basename(pluginPath)}" at path ${pluginInstallationPath}`);
            return FileType.fromFile(pluginPath).then(fileType => {
                    if (fileType.mime) {
                        if (fileType.mime === 'application/zip') {
                            const zip = new AsyncZip({ file: pluginPath });
                            return zip.extract(null, pluginInstallationPath).then(() => zip.close());
                        } else if (fileType.mime === 'application/gzip') {
                            // By setting strip 1 we disabled extracting into subdirectory within the target path
                            // For tar we have to ensure the target directory exists already
                            return mkdir(pluginInstallationPath, { recursive: true }).then(() => tar.extract({ file: pluginPath, cwd: pluginInstallationPath, strip: 1 }));
                        }
                    }
                    throw new Error(`Unsupported archive: "${pluginPath}"`);
                });
        });
}
...
function installCamundaModeler (config) {
    ...
    return checkExistingInstallation(config.getInstallationPath(), config.shouldOverwriteExistingInstallation())
        ...
        .then(({ modeler, plugins }) => Promise.all([
            extractTo(modeler, config.getInstallationPath()),
            ...plugins.map(plugin => extractPluginTo(config.getInstallationPath(), plugin)),
